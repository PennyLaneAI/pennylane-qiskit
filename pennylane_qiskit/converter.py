...
refferal_to_forum = "If you are experiencing difficulties with converting circuits from Qiskit, you can reach out " \
...
def _format_params_dict(quantum_circuit, params, *args, **kwargs):
    ...
    for k, v in kwargs.items():
        ...
        if not qc_param:
            ...
            raise TypeError(
                f"Got unexpected parameter keyword argument '{k}'. Circuit contains parameters: {param_names}. {refferal_to_forum}"
            )
        params[qc_param[0]] = v
    ...
    if len(args) > len(arg_parameters):
        raise TypeError(
            f"Expected {len(arg_parameters)} positional argument{'s' if len(arg_parameters) > 1 else ''} but {len(args)} were given. {refferal_to_forum}"
        )
...
def _check_circuit_and_assign_parameters(
    ...
    for name in ["wires", "params"]:
        if name in [p.name for p in quantum_circuit.parameters]:
            raise RuntimeError(
                ...
                f"quantum function, as this argument is reserved. {refferal_to_forum}"
            )
    ...
    if params is None:
        if quantum_circuit.parameters:
            ...
            param_names = ", ".join([p.name for p in quantum_circuit.parameters])
            raise TypeError(
                f"Missing required argument{s} to define Parameter value{s} for: {param_names}. {refferal_to_forum}"
            )
    ...
    if undefined_params:
        ...
        raise TypeError(
            f"Missing {len(undefined_params)} required argument{s} to define Parameter value{s} for: {param_names}. {refferal_to_forum}"
        )
