...
def _format_params_dict(quantum_circuit, params, *args, **kwargs):
    ...
    for k, v in kwargs.items():
        ...
        if not qc_param:
            param_names = ", ".join([p.name for p in expected_params])
            raise TypeError(
    ...
    expected_arg_params = [getattr(p, "vector", p) for p in expected_params if p.name not in kwargs.keys()]
    ...
    if len(args) > len(expected_arg_params):
        param_vector_info = "Note that PennyLane expects to recieve a ParameterVector as a single argument " \
                            "containing all ParameterVectorElements." if has_param_vectors else ""
        raise TypeError(
