...
def qiskit_session(device, **kwargs):
    ...
    Args:
        ...
        **kwargs: 
            keyword arguments for session settings. At the time of writing, the only relevant
            keyword argument is "max_time", which lets you set the maximum amount of time the session
            is open. For the most up-to-date information, please refer to the Qiskit Session
            `documentation <https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/qiskit_ibm_runtime.Session>`_.
