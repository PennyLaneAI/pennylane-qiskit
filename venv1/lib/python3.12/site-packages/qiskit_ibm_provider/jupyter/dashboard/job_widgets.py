...
def make_clear_button(watcher: "IBMDashboard") -> widgets.GridBox:
    ...
    clear = widgets.Button(
        ...
        layout=widgets.Layout(
            width="70px", grid_area="right", padding="0px 0px 0px 0px"
        ),
    )
...
def make_labels() -> widgets.HBox:
    ...
    Returns:
        The labels widget.
    """
    labels0 = widgets.HTML(
        value="<h5>Job ID</h5>", layout=widgets.Layout(width="190px")
    )
    labels1 = widgets.HTML(
        value="<h5>Backend</h5>", layout=widgets.Layout(width="165px")
    )
    labels2 = widgets.HTML(
        value="<h5>Status</h5>", layout=widgets.Layout(width="125px")
    )
    labels3 = widgets.HTML(
...
def create_job_widget(
    watcher: "IBMDashboard",
    job: IBMJob,
    backend: str,
    status: str = "",
    queue_pos: Optional[int] = None,
    est_start_time: Optional[datetime] = None,
) -> widgets.HBox:
    ...
    backend_label = widgets.HTML(
        value="{}".format(backend), layout=widgets.Layout(width="165px")
    )

    queue_str = " ({})".format(queue_pos) if status == "QUEUED" and queue_pos else ""
    status_label = widgets.HTML(
        value="{}{}".format(status, queue_str), layout=widgets.Layout(width="125px")
    )

    est_time = est_start_time.strftime("%H:%M %Z (%m/%d)") if est_start_time else "-"
    est_time_label = widgets.HTML(
        value="{}".format(est_time), layout=widgets.Layout(width="125px")
    )

    close_button = widgets.Button(
        button_style="",
        icon="close",
        layout=widgets.Layout(width="30px", margin="0px 5px 0px 0px"),
    )
    ...
    job_grid = widgets.HBox(
        children=[close_button, id_label, backend_label, status_label, est_time_label],
        layout=widgets.Layout(min_width="690px", max_width="690px"),
    )
