...
def make_backend_widget(backend_item: BackendWithProviders) -> wid.HBox:
    ...
    left_values = wid.VBox(
        children=[qubits_val_wid, status_val_wid],
        layout=wid.Layout(margin="1px 0px 0px 0px"),
    )

    left_stats = wid.HBox(
        children=[left_labels, left_values], layout=wid.Layout(width="175px")
    )

    version_wid = wid.HTML(value=STAT_FONT_TITLE.format("Version:"))
    version_val_wid = wid.HTML(
        value=STAT_FONT_VALUE.format(config.backend_version),
        layout=wid.Layout(margin="3px 0px 0px 0px"),
    )

    queue_wid = wid.HTML(value=STAT_FONT_TITLE.format("Queue:"))
    queue_val_wid = wid.HTML(
        value=STAT_FONT_VALUE.format(status.pending_jobs),
    ...
    right_stats = wid.HBox(
        children=[right_labels, right_values],
        layout=wid.Layout(width="auto", margin="0px 0px 0px 20px"),
    )
    ...
    device_stats = wid.VBox(
        ...
        layout=wid.Layout(width="auto", margin="0px 20px 0px 0px"),
    )
    ...
    t12_str = (
        "<font size='3' face='monospace'>{t1}/{t2}</font><font size='2'> {units}</font>"
    )
    ...
    qv_wid = wid.HTML(
        value=qv_str.format(qv=quant_vol), layout=wid.Layout(margin="-1px 0px 0px 0px")
    )
    ...
    left_wid = wid.VBox(children=left_wids, layout=wid.Layout(margin="2px 0px 0px 0px"))
    right_wid = wid.VBox(
        children=right_wids, layout=wid.Layout(margin="0px 0px 0px 3px")
    )
    ...
    out = wid.HBox(
        children=[device_stats, gate_map],
        layout=wid.Layout(
            width="auto",
            height="auto",
