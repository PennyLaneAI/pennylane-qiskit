...
def gates_tab(backend: BackendV2) -> wid.GridBox:
    ...
    header_html = header_html.format(key="last_update_date", value=date_str)
...
tr:nth-child(even) {background-color: #f6f6f6;};
    ...
    middle_table_widget = wid.HTML(
        value=middle_table, layout=wid.Layout(grid_area="middle")
    )
    right_table_widget = wid.HTML(
