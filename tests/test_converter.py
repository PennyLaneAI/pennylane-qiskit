...
class TestLoadNoiseModel:
    ...
    def test_build_noise_model(self):
        ...
        from pennylane_qiskit.noise_models import _kraus_to_choi
        ...
        for (pl_k, pl_v), (qk_k, qk_v) in zip(
            ...
            if not isinstance(pl_op, qml.QubitChannel):
                assert np.allclose(pl_op.data, qk_op.data)
            else:
                choi_mat1 = _kraus_to_choi(Kraus(qk_op.data[0]))
                choi_mat2 = _kraus_to_choi(Kraus(pl_op.data[0]))
